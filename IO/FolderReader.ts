...
export class FolderReader extends Reader {
	...
	get readable(): boolean {
		return (
			(this.current != undefined && this.current.readable) ||
			this.files.length > 0
		);
	}
	get opened(): boolean {
	...
	private lastLocation: Error.Location = new Error.Location(
		Uri.Locator.empty,
		0,
		0
	);
			...
			if (ignoreFiles.indexOf(filename) == -1) {
				if (fs.lstatSync(filename).isDirectory())
					result = result.concat(
						FolderReader.getFiles(filename, filetype, ignoreFiles)
					);
				else if (
					file.length > filetype.length &&
					file.lastIndexOf(filetype, file.length - filetype.length) ===
						file.length - filetype.length
				)
	...
	static open(resource: Uri.Locator): Reader | undefined {
		...
		try {
			if (
				(resource &&
					(resource.scheme.length == 0 ||
						(resource.scheme.length == 1 && resource.scheme[0] == "file")) &&
					resource.isFolder) ||
				resource.name.match("*")
			)
				files = FolderReader.getFiles(
					(resource.isRelative ? "" : path.sep) +
						resource.folder.path.join(path.sep),
					resource.extension
				);
		} catch (error) {}
		return files
			? new FolderReader(
					files.map(
						(f) => new Uri.Locator(["file"], undefined, f.split(path.sep))
					)
			  )
			: undefined;
	}
