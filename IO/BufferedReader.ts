...
export class BufferedReader extends Reader {
	...
	get isEmpty(): boolean {
		return (
			(this.buffer.length == 0 || this.buffer[0].data == "\0") &&
			this.backend.isEmpty
		);
	}
	...
	readIf(value: string | string[]): string | undefined {
		...
		if (value)
			if (typeof value == "string")
				result =
					(this.peek(value.length) == value && this.read(value.length)) ||
					undefined;
			else if (
				value.length > 0 &&
				!(result = this.readIf(value[0])) &&
				value.length
			)
